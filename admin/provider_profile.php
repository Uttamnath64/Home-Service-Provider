...
<body>
  ...
  function all_c($conn,$status='')
  {
    if(empty($status))
    {
    $sql = "SELECT count(id) as total FROM complain  WHERE Provider_id = '$_GET[id]' ";
    }
    else{
    $sql = "SELECT count(id) as total FROM complain  WHERE Provider_id = '$_GET[id]' and Status='$status'";
     } 
    $result = mysqli_query($conn, $sql);
...
?>
	<div style="margin-top: 20px;" class="text-center mb-20">
      <h1 class="sm:text-3xl text-2xl font-medium text-center title-font text-gray-900 mb-4">User Profile @<?=$namep?></h1>
    </div>
    ...
    <div class="flex flex-wrap -m-4">
      <div style="text-align: left; width: 100%;" class="p-4 lg:w-1/2 md:w-full">
        <div class="flex border-2  rounded-lg border-gray-200 border-opacity-50 p-8 sm:flex-row flex-col">
          ...
          <div class="flex-grow">
            <h2 class="text-gray-900 text-lg title-font font-medium"><?=$namep ?></h2>
            <p class="leading-relaxed text-base"><?=$emailp?></p>
...
<section style="margin-left: 170px; margin-right: 170px; margin-top: -160px; margin-bottom: -105px;" class="text-gray-600 body-font">
  <div class="container px-5 py-24 mx-auto">
    ...
    <div class="flex flex-wrap lg:w-4/5 sm:mx-auto sm:mb-2 -mx-2">
    	<a class="p-2 sm:w-1/2 w-full" href="provider_profile_view.php?id=<?=$_GET['id']?>">
      ...
      <a class="p-2 sm:w-1/2 w-full" href="complain_provider.php?id=<?=$_GET['id']?>">
      <div style="width: 100%;" class="p-2 sm:w-1/2 w-full">
        <div class="bg-gray-100 rounded flex p-4 h-full items-center">
          <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="3" class="text-yellow-500 w-6 h-6 flex-shrink-0 mr-4" viewBox="0 0 24 24">
            <path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path>
      ...
      <a class="p-2 sm:w-1/2 w-full" href="feedback_provider.php?id=<?=$_GET['id']?>">
      <div style="width: 100%;" class="p-2 sm:w-1/2 w-full">
        <div class="bg-gray-100 rounded flex p-4 h-full items-center">
          <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="3" class="text-yellow-500 w-6 h-6 flex-shrink-0 mr-4" viewBox="0 0 24 24">
            <path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path>
      ...
      <a class="p-2 sm:w-1/2 w-full" href="user_remove.php?id=<?=$_GET['id']?>&c=p">
      <div style="width: 100%;" class="p-2 sm:w-1/2 w-full">
        <div class="bg-gray-100 rounded flex p-4 h-full items-center">
          <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="3" class="text-yellow-500 w-6 h-6 flex-shrink-0 mr-4" viewBox="0 0 24 24">
            <path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path>
