...
if (isset($_POST['submit'])) {
  $sql = "SELECT * FROM provider_data  WHERE Mobile_Number = '$_POST[mobile_number]' and Password = '$_POST[password]'";  
  $result = mysqli_query($conn, $sql);    
  $count = mysqli_num_rows($result); 
  if($count == 1)
  {  
    $sql_data = "SELECT * FROM provider_data  WHERE Mobile_Number = '$mobilenumber'";
    $result = mysqli_query($conn, $sql);
...
?>
  ...
  <div class="container px-5 py-10 mx-auto flex flex-wrap items-center">
    <div class="lg:w-3/5 md:w-1/2 md:pr-16 lg:pr-0 pr-0">
      ...
      <p class="leading-relaxed mt-4">Home Service Provider is much better when sign in to account.</p>
    ...
    <div class="lg:w-2/6 md:w-1/2 bg-gray-100 rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0">
      ...
      <div style="display: flex; text-align: center;"><p class="text-xs text-gray-500 mt-3">Don't have an account?</p><a href="signup.php" class="text-xs text-yellow-500 mt-3 px-4" href="#">Sign Up</a></div>
    </div>
