...
<body>
  ...
  function all_c($conn,$status='')
  {
    if(empty($status))
    {
    $sql = "SELECT count(id) as total FROM complain  WHERE User_id = '$_SESSION[id]' ";
    }
    else{
    $sql = "SELECT count(id) as total FROM complain  WHERE User_id = '$_SESSION[id]' and Status='$status'";
     } 
    $result = mysqli_query($conn, $sql);
